...
本分支固定安装 `vendor/windchime-embed-0.6.0.tgz`，与 UliUli 使用同一压缩包。共享库源码在 WindChime 仓库；本网站提供登录、页面和数据库连接，不维护另一套播出审核规则。
...
```powershell
$env:DATABASE_PATH = "$env:TEMP\windchime-mia-test\codes.db"
$env:WINDCHIME_MEDIA_DIRECTORY = "$env:TEMP\windchime-mia-test\media"
...
$env:WINDCHIME_SITE_ORIGIN = "http://localhost:3012"
npm ci
...
启动 WindChime 的桌面应用，填写 `http://localhost:3012` 完成浏览器配对即可，不需要另外启动网关。桌面运行及打包方式见共享仓库的 `apps/desktop/README.md`。
