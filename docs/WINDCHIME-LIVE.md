...
本分支固定安装 `vendor/windchime-embed-0.8.3-c4efb3dd2d7dab7fa432bc29350f0b3ed74b6a58.tgz`，与 UliUli 使用同一压缩包及对应 lockfile。共享库维护信件、话题、审核、授权和数据库规则；网站保留原有 `/mail` 界面，Windows 桌面提供完整信箱管理和独立直播展示窗口。

[0.8.3 升级与本轮验证](WINDCHIME-083-UPGRADE.md) 记录本次精确包摘要、隔离构建、重复迁移和 HTTP 回归；旧版本报告保留原日期及范围。

Mia 本轮未部署生产网站；本地构建、测试和依赖升级不代表线上已更新。
...
- `WINDCHIME_SITE_ORIGIN`：网站公开 Origin，例如 `https://your-mia-site.example`，反向代理部署应明确设置。
...
```powershell
$env:DATABASE_PATH = "$env:TEMP\windchime-mia-080-test\codes.db"
$env:WINDCHIME_MEDIA_DIRECTORY = "$env:TEMP\windchime-mia-080-test\media"
...
$env:WINDCHIME_SITE_ORIGIN = "http://localhost:3032"
$env:WINDCHIME_BUILD_DIRECTORY = ".windchime-080-test"
npm ci
npm run dev -- --hostname 127.0.0.1 --port 3032
```

打开 `http://localhost:3032/mail` 登录、生成全站密钥并连接桌面。迁移回归使用 `npm run test:mail:migrations`；原信箱 HTTP 回归使用 `MAIL_SMOKE_ALLOW_WRITES=1`、`MAIL_SMOKE_BASE_URL`、`MAIL_SMOKE_PASSWORD` 后运行 `npm run test:mail`，仅用于隔离 localhost 合成数据。
