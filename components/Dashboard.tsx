...
// --- 1. Hero ---
const HeroSection: React.FC<{ stats: string | number; config: RuntimeSiteConfig }> = ({ stats, config }) => {
    ...
    <div className="h-screen w-full flex flex-col justify-center items-center relative px-4 md:px-6 mb-12 md:mb-20 overflow-hidden">
      <div className="z-10 text-center relative w-full max-w-[90vw]">
        ...
        <div className="mt-12 flex gap-4 justify-center">
          ...
          >
            <span className="relative z-10">{cfg.startBtn}</span>
          </button>
        </div>
      </div>
...
// --- 2. Model Breakdown ---
const ModelBreakdown: React.FC<{ avatarSrc: string; config: RuntimeSiteConfig; assetVersion?: number }> = ({
  avatarSrc,
  config,
  assetVersion,
}) => {
  const cfg = config.model;
  const expressions: ExpressionItem[] = cfg.expressions || [];

  const [selected, setSelected] = useState(0);
  const safeIndex = expressions.length > 0 ? Math.min(selected, expressions.length - 1) : 0;
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="flex items-end justify-between mb-10 border-b border-(--mia-warm-grey) pb-6">
          <h2 className="font-display text-2xl md:text-4xl font-bold text-(--mia-ink)">
            {cfg.titlePrefix}
            <span className="text-(--mia-gold)">{cfg.titleSuffix}</span>
          </h2>
            ...
            <div className="relative w-full h-full flex items-center justify-center py-4">
              <AnimatePresence mode="wait">
                {fullSrc ? (
                  ...
                  >
                    <div className="font-display text-sm tracking-[0.3em]">{cfg.imagePlaceholderTitle}</div>
                    <div className="font-serif-cn text-xs text-(--mia-warm-grey-deep)">
                      {current?.id ? `${current.id} · ${cfg.imageMissingText}` : cfg.imageMissingText}
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
...
const LiveStatusSection: React.FC<{
  ...
  const getColorClass = (color: string) => {
    if (color === 'gold')
      return {
        border: 'group-hover:border-(--mia-gold)',
        text: 'text-(--mia-gold-deep)',
        glow: 'group-hover:shadow-[0_0_20px_rgba(196,169,110,0.3)]',
      };
    if (color === 'blush')
      return {
        border: 'group-hover:border-(--mia-blush)',
        text: 'text-(--mia-blush)',
        glow: 'group-hover:shadow-[0_0_20px_rgba(232,201,184,0.3)]',
      };
    if (color === 'ink')
      return {
        border: 'group-hover:border-(--mia-ink)',
        text: 'text-(--mia-ink)',
          ...
          <GlassCard className="p-6 md:p-8 flex flex-col justify-center relative overflow-hidden group min-h-50">
            <div className="relative z-10">
              ...
              <p className="font-serif-cn text-(--mia-gold-deep) text-sm tracking-widest mb-6 opacity-80">
                {cfg.roomPrefix} {cfg.roomId}
              </p>
              <a
                href={`https://live.bilibili.com/${cfg.roomId}`}
                target="_blank"
                rel="noreferrer"
                className={`
                ...
                `}
              >
                {liveStatus ? cfg.liveNowText : cfg.offlineText}
              </a>
            </div>
          </GlassCard>
            ...
            <div className="flex flex-col gap-3 h-full">
              <div>
                <h3 className="font-display text-lg font-bold text-(--mia-ink) mb-2 flex items-center gap-2 group-hover:text-(--mia-gold-deep) transition-colors">
                  <Icons.Calendar size={18} /> {cfg.scheduleTitle}
                </h3>
                <div className="text-xs text-(--mia-gold-deep) font-serif-cn space-y-1 pl-1">
                  <div className="flex justify-between">
                    <span>{cfg.schedule.morning}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>{cfg.schedule.evening}</span>{' '}
...
export const HorizontalVideoGallery: React.FC<{ videos: VideoItem[]; config: RuntimeSiteConfig }> = ({ videos, config }) => {
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    if (!videos || videos.length === 0 || !wrapper || !container || !pin) return;

    const getScrollDistance = () => Math.max(0, wrapper.scrollWidth - window.innerWidth);
    ...
    return () => {
      try {
        ...
        console.warn('[HorizontalVideoGallery] ScrollTrigger cleanup skipped:', error);
      } finally {
