...
export const ThreeBackground: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (canvas) {
      ...
      // Mia 暖光云层雾（奶白调）
      scene.fog = new THREE.FogExp2(0xfbf6ec, 0.002);
      ...
      // Mia 香槟金 / 暖肤粉 / 乳白
      const color1 = new THREE.Color(0xc4a96e);
      const color2 = new THREE.Color(0xe8c9b8);
  ...
  return (
    <div className="fixed inset-0 w-full h-full -z-10 bg-(--mia-cream, #fbf6ec) overflow-hidden">
      <div ref={bgWrapperRef} className="absolute inset-0 w-full h-full opacity-0 pointer-events-none">
        ...
        />
        {/* Mia 主题：背景覆盖仅需轻薄暖调，不再压走塔色 */}
        <div className="absolute inset-0 bg-(--mia-cream)/30"></div>
      </div>
  ...
  useEffect(() => {
    ...
    const hoverStart = (e: MouseEvent) => {
      ...
      if (target && target.closest("a, button, .interactive")) {
        ...
        cursor.style.backgroundColor = "rgba(196, 169, 110, 0.25)";
    ...
    const hoverEnd = () => {
      ...
      cursor.style.borderColor = "var(--mia-gold)";
    };
