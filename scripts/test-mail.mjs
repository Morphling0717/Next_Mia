...
const base = process.env.MAIL_SMOKE_BASE_URL || "http://localhost:3012";
if (
...
const response = await fetch(sessionUrl, {
  ...
  body: JSON.stringify({ scope: "mail", password }),
});
...
try {
  ...
  console.log("Next_Mia: 0.8 six layouts, fixed image allocation, shared state and no automatic output passed.");
  ...
  console.log(
    "Next_Mia: original session + legacy auth, public DTO/SSR privacy, submission, inbox filters, topic isolation, favorite, archive/restore, review, batches, block/unblock and settings passed.",
  );
} finally {
